...
export const PdfExportService = {
  ...
  _drawCell(pdf: jsPDF, text: string, x: number, y: number, width: number, align: 'left' | 'center' = 'center', isBold = false) {
  ...
  async generateAndDownload(state: AppState, activeDayIdx: number, filename: string): Promise<void> {
    ...
    roomsWithExams.forEach((group, gIdx) => {
      ...
      group.exams.forEach((exam, eIdx) => {
        ...
        if (zebra) { pdf.setFillColor(248, 250, 252); pdf.rect(PDF_CONFIG.marginX, currentY, contentWidth, PDF_CONFIG.rowHeight, 'F'); }
        pdf.setDrawColor(0);
        pdf.setLineWidth(0.1);
        ...
        let x = PDF_CONFIG.marginX;
        pdf.setFontSize(9.5);
        this._drawCell(pdf, minToTime(examSlotToMin(exam.startTime)), x, currentY, colWidths[0], 'center', true); x += colWidths[0];
        pdf.line(x, currentY, x, currentY + PDF_CONFIG.rowHeight);
        pdf.setFontSize(9);
        this._drawCell(pdf, state.rooms.find(r => r.id === exam.prepRoomId)?.name || '-', x, currentY, colWidths[1]); x += colWidths[1];
        pdf.line(x, currentY, x, currentY + PDF_CONFIG.rowHeight);
        pdf.setFontSize(9);
        this._drawCell(pdf, group.room.name, x, currentY, colWidths[2]); x += colWidths[2];
        pdf.line(x, currentY, x, currentY + PDF_CONFIG.rowHeight);
        pdf.setFontSize(9.5);
        this._drawCell(pdf, state.students.find(s => s.id === exam.studentId)?.lastName || '???', x, currentY, colWidths[3], 'left', true); x += colWidths[3];
        pdf.line(x, currentY, x, currentY + PDF_CONFIG.rowHeight);
        pdf.setFontSize(8.5);
        const isComb = state.subjects.find(s => s.name === exam.subject)?.isCombined;
        this._drawCell(pdf, `${exam.subject}${isComb ? '*' : ''}`, x, currentY, colWidths[4], 'left'); x += colWidths[4];
        pdf.line(x, currentY, x, currentY + PDF_CONFIG.rowHeight);
        ...
        this._drawCell(pdf, isComb ? `${t}/${p}` : t, x, currentY, colWidths[5]); x += colWidths[5];
        pdf.line(x, currentY, x, currentY + PDF_CONFIG.rowHeight);
        this._drawCell(pdf, isComb ? `${t}/${p}` : p, x, currentY, colWidths[6]); x += colWidths[6];
        pdf.line(x, currentY, x, currentY + PDF_CONFIG.rowHeight);
  ...
  async generatePrepRoomPdf(state: AppState, activeDayIdx: number, filename: string): Promise<void> {
    ...
    prepRoomIds.forEach((prepRoomId, pageIdx) => {
      ...
      currentY = this._drawHeader(pdf, `Mündliches Abitur: `, `Vorbereitungsraum ${room?.name || '-'}`, dateStr);
      ...
      sortedExams.forEach((exam, eIdx) => {
        ...
        pdf.setFont('helvetica', 'bold');
        pdf.text(minToTime(examSlotToMin(exam.startTime) - 20), cellX + colWidths[0] / 2, currentY + (PDF_CONFIG.rowHeight/2), { align: 'center', baseline: 'middle' });
        cellX += colWidths[0]; 
        pdf.setLineWidth(0.1);
        ...
        pdf.text(`${s?.lastName || '???'}, ${s?.firstName || ''}`, cellX + 3, currentY + (PDF_CONFIG.rowHeight/2), { baseline: 'middle' });
        cellX += colWidths[1]; 
        pdf.line(cellX, currentY, cellX, currentY + PDF_CONFIG.rowHeight);
        const t = state.teachers.find(t => t.id === exam.teacherId);
        pdf.text(t?.lastName || '-', cellX + 3, currentY + (PDF_CONFIG.rowHeight/2), { baseline: 'middle' });
        cellX += colWidths[2]; 
        pdf.line(cellX, currentY, cellX, currentY + PDF_CONFIG.rowHeight);
        pdf.text(exam.subject, cellX + 3, currentY + (PDF_CONFIG.rowHeight/2), { baseline: 'middle' });
        currentY += PDF_CONFIG.rowHeight;
      });
