...
export const DATA_KEYS = {
  TEACHERS: 'lx_teachers', STUDENTS: 'lx_students', ROOMS: 'lx_rooms',
  DAYS: 'lx_days', SUBJECTS: 'lx_subjects', EXAMS: 'lx_exams',
  SUPERVISIONS: 'lx_supervisions', COLLECTED: 'lx_collected', 
  SETTINGS: 'lx_settings', HISTORY: 'lx_history'
};
...
export const setupAuth = async (password: string): Promise<CryptoKey> => {
  ...
  const verifyContent = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, new TextEncoder().encode('VERIFY'));
...
export const verifyAndGetKey = async (password: string): Promise<CryptoKey | null> => {
  ...
  try {
    ...
    const decrypted = await crypto.subtle.decrypt(
      ...
      key, fromBase64(meta.verifyCipher)
    );
...
export const saveEncryptedState = async (state: AppState, key: CryptoKey, specificKey?: string): Promise<void> => {
  ...
  const mapping: Record<string, string> = {
    teachers: DATA_KEYS.TEACHERS, students: DATA_KEYS.STUDENTS, rooms: DATA_KEYS.ROOMS,
    days: DATA_KEYS.DAYS, subjects: DATA_KEYS.SUBJECTS, exams: DATA_KEYS.EXAMS,
    supervisions: DATA_KEYS.SUPERVISIONS, collectedExamIds: DATA_KEYS.COLLECTED, 
    settings: DATA_KEYS.SETTINGS, historyLogs: DATA_KEYS.HISTORY
  };
...
export const loadEncryptedState = async (key: CryptoKey): Promise<Partial<AppState>> => {
  const decrypt = async (dbKey: string) => {
    ...
    if (!item) return null;
    try {
      const dec = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: fromBase64(item.iv) }, key, fromBase64(item.ciphertext));
  ...
  return {
    teachers: r[0] || [], students: r[1] || [], rooms: r[2] || [],
    days: r[3] || [], subjects: r[4] || [], exams: r[5] || [],
    supervisions: r[6] || [], collectedExamIds: r[7] || [],
...
export const decryptFromFile = async (buffer: ArrayBuffer, password: string): Promise<AppState> => {
  ...
  const dec = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: fromBase64(payload.iv) }, key, fromBase64(payload.ciphertext));
  return JSON.parse(new TextDecoder().decode(dec));
};

export const clearDatabase = async (): Promise<void> => { await localforage.clear(); };
