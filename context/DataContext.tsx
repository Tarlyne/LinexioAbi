...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateTeacher = useCallback((t: Teacher) => setTeachers(prev => prev.map(curr => curr.id === t.id ? t : curr)), []);
  const deleteTeacher = useCallback((id: string) => setTeachers(prev => prev.filter(t => t.id !== id)), []);

  const upsertTeachers = useCallback((newList: Teacher[]) => setTeachers(prev => {
  ...
  const updateStudent = useCallback((s: Student) => setStudents(prev => prev.map(curr => curr.id === s.id ? s : curr)), []);
  const deleteStudent = useCallback((id: string) => setStudents(prev => prev.filter(s => s.id !== id)), []);

  const upsertStudents = useCallback((newList: Student[]) => setStudents(prev => {
  ...
  const updateRoom = useCallback((r: Room) => setRooms(prev => prev.map(curr => curr.id === r.id ? r : curr)), []);
  const deleteRoom = useCallback((id: string) => setRooms(prev => prev.filter(r => r.id !== id)), []);

  const upsertRooms = useCallback((newList: Room[]) => setRooms(prev => {
  ...
  const addDay = useCallback((d: ExamDay) => setDays(prev => [...prev, d].sort((a,b) => a.date.localeCompare(b.date))), []);
  const updateDay = useCallback((d: ExamDay) => setDays(prev => prev.map(curr => curr.id === d.id ? d : curr).sort((a,b) => a.date.localeCompare(b.date))), []);
  const deleteDay = useCallback((id: string) => setDays(prev => prev.filter(d => d.id !== id)), []);

  const addSubject = useCallback((s: Subject) => setSubjects(prev => [...prev, s].sort((a,b) => a.name.localeCompare(b.name, 'de'))), []);
