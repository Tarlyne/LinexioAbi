...
export const ProtocolView: React.FC = () => {
  ...
  const filteredBlocks = protocolBlocks.filter(b => showCollected || !b.isCollected || exitingIds.includes(b.lastExamId));
            ...
            return (
              <div 
                ...
                className={`glass-nocturne border p-4 flex items-center justify-between transition-all duration-300 ${
                  ...
                  block.isCollected ? 'opacity-40 border-slate-800 bg-slate-900/20 shadow-none' : 
                  isActuallyLate ? 'border-red-500/50 bg-red-500/5 shadow-[0_0_20px_rgba(239,68,68,0.15)]' : 
                  isActuallyReady ? 'border-indigo-500/50 bg-indigo-500/10 shadow-[0_0_20px_rgba(99,102,241,0.15)]' : 
                  'border-slate-700/50 bg-slate-900/40'
                }`}
              >
                <div className="flex items-center gap-6 min-w-0 flex-1 mr-4">
                  <div className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center border font-black transition-all shrink-0 ${
                     isActuallyLate ? 'bg-red-500/20 border-red-500/40 text-red-400 shadow-[0_0_10px_rgba(239,68,68,0.2)]' :
                     isActuallyReady ? 'bg-indigo-500/20 border-indigo-500/40 text-indigo-300 shadow-[0_0_10px_rgba(99,102,241,0.2)]' :
                  ...
                  <div className="space-y-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <Clock size={14} className={isActuallyLate ? 'text-red-500' : isActuallyReady ? 'text-indigo-400' : 'text-slate-500'} />
                      <span className={`text-xl font-black tracking-tight ${
                         isActuallyLate ? 'text-red-400' : isActuallyReady ? 'text-indigo-200' : block.isCollected ? 'text-slate-500' : 'text-slate-300'
                      }`}>
                        {block.pickupTimeStr} Uhr
                      </span>
                      {isActuallyLate && (
                        <span className="badge badge-red ml-2 animate-pulse shadow-[0_0_10px_rgba(239,68,68,0.3)]">Überfällig</span>
                      )}
                      {isActuallyReady && (
                        <span className="badge badge-indigo ml-2 shadow-[0_0_10px_rgba(99,102,241,0.3)]">Abholbereit</span>
                      )}
                    </div>
