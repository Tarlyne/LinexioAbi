...
export const DataList: React.FC<DataListProps> = ({ activeTab, data, onEdit, exitingId }) => {
  ...
  const columns: Column<any>[] = [
    { 
      header: '#', key: 'index', width: 'w-14', align: 'center', 
      render: (_, idx) => <span className="text-[10px] font-mono text-slate-500">{idx + 1}</span> 
    },
    { 
      header: activeTab === 'rooms' ? 'Raum' : activeTab === 'days' ? 'Datum' : activeTab === 'subjects' ? 'Fach' : 'Nachname',
      key: 'lastName',
      render: (item) => (
  ...
  if (activeTab === 'rooms') {
    columns.push({ 
      ...
      key: 'type' as any,
      render: (item) => (
        <span className={`badge ${getRoomBadgeClass(item.type)}`}>
          {item.type}
        </span>
      )
    });
  ...
  columns.push({
    header: 'Aktion', key: 'action', width: 'w-20', align: 'center',
    render: (item) => (
      <button onClick={() => onEdit(item)} className="p-1.5 text-slate-500 hover:text-cyan-400 hover:bg-cyan-500/10 rounded-lg transition-all">
        <Settings size={18} />
      </button>
    )
  });
        ...
        <table className="w-full text-left border-collapse table-fixed">
          <thead className="sticky top-0 bg-slate-900/95 backdrop-blur-md z-10 shadow-sm">
            <tr className="border-b border-slate-700/50">
              {columns.map(col => (
                <th key={col.header} className={`${col.width || ''} px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-widest ${col.align === 'center' ? 'text-center' : ''}`}>
                  {col.header}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-slate-800/40">
            {data.map((item, idx) => (
              <tr key={item.id} className={`hover:bg-cyan-500/5 transition-colors group text-sm ${exitingId === item.id ? 'animate-row-exit' : ''}`}>
                {columns.map(col => (
                  <td key={`${item.id}-${col.header}`} className={`px-4 py-3 truncate ${col.align === 'center' ? 'text-center' : ''}`}>
            ...
            {data.length === 0 && (
              <tr>
                <td colSpan={columns.length} className="px-4 py-20 text-center text-slate-600 italic text-sm">
