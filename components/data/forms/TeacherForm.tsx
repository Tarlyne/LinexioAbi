      ...
      <div className="grid grid-cols-2 gap-5 items-start">
        ...
        <div className="space-y-3 pt-2">
           <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <div className={`w-6 h-6 rounded-lg border flex items-center justify-center transition-all ${formData.isPartTime ? 'bg-cyan-600 border-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.3)]' : 'bg-slate-900/50 border-slate-700 group-hover:border-slate-500'}`}>
           ...
           <label className="flex items-center gap-3 cursor-pointer group">
              <input type="checkbox" checked={formData.isLeadership || false} onChange={e => updateField('isLeadership', e.target.checked)} className="sr-only" />
              <div className={`w-6 h-6 rounded-lg border flex items-center justify-center transition-all ${formData.isLeadership ? 'bg-amber-600 border-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.3)]' : 'bg-slate-900/50 border-slate-700 group-hover:border-slate-500'}`}>
                {formData.isLeadership && <ShieldCheck size={14} className="text-white" />}
              </div>
              <span className="text-xs font-semibold text-slate-300 group-hover:text-white">Schulleitung</span>
           </label>
        </div>
      </div>
