  ...
  return (
    <aside className="w-full lg:w-80 space-y-4 shrink-0 overflow-y-auto no-scrollbar pb-6">
      {(activeTab === 'teachers' || activeTab === 'students') && (
        <div className="glass-nocturne p-5 border border-slate-700/30">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-8 h-8 bg-cyan-500/10 rounded-lg flex items-center justify-center text-cyan-400 shrink-0"><Upload size={18} /></div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={stat.label} className="flex items-center justify-between p-3 bg-slate-900/40 rounded-xl border border-slate-800/50">
                ...
                <span className={`text-sm font-bold px-2 py-0.5 rounded border min-w-[2rem] text-center ${stat.color}`}>{stat.val}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {activeTab === 'days' && (
        <div className="glass-nocturne p-5 border border-slate-700/30">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-8 h-8 bg-cyan-500/10 rounded-lg flex items-center justify-center text-cyan-400 shrink-0"><Calendar size={18} /></div>
            <h3 className="font-bold text-white text-sm">Zeitliche Basis</h3>
          </div>
          <p className="text-xs text-slate-400 leading-relaxed">Das Grid passt sich automatisch an die hier definierten Tage an.</p>
        </div>
      )}
      ...
      {activeTab === 'subjects' && (
        <div className="glass-nocturne p-5 border border-slate-700/30">
          ...
          <div className="p-3 bg-slate-900/40 rounded-xl border border-slate-800/50 flex justify-between items-center">
             ...
             <span className="text-sm font-bold text-cyan-400 px-2 py-0.5 rounded border border-cyan-500/20 bg-cyan-500/5">{stats.subjectsCount}</span>
          </div>
