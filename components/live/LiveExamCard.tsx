...
export const LiveExamCard = React.memo(({
  exam, student, room, prepRoom, status, isUrgent, examTimeStr, prepTimeStr, canComplete, onTogglePresence, onComplete
}: LiveExamCardProps) => {
  ...
  return (
    <div className={`glass-nocturne border transition-all duration-300 flex flex-col overflow-hidden shadow-xl border-slate-700/50 hover:bg-slate-800/40`}>
      {/* Header Section (Pure Text Labels) */}
      <div className={`px-4 py-2 flex justify-between items-center ${
        status.phase === 'TAXI_TO_PREP' ? 'bg-amber-500/15' : 
        status.phase === 'TAXI_TO_EXAM' ? 'bg-cyan-500/15' : 'bg-slate-900/60'
      }`}>
        <div className="flex items-center gap-2 overflow-hidden">
          ...
          <div className="flex items-baseline gap-2 truncate">
            <span className={`text-[10px] font-black uppercase tracking-wider ${getStatusTextColor()} ${status.isBlinking ? 'animate-pulse' : ''}`}>
              {status.label}
            </span>
            {status.countdown && (
      ...
      <div className="p-4">
        <div className="grid grid-cols-[auto_1fr_auto] gap-x-5 gap-y-1 items-center">
          <span className="text-[8px] font-bold text-slate-500 uppercase tracking-widest text-center">Vorb.</span>
          <div />
          <span className="text-[8px] font-bold text-slate-500 uppercase tracking-widest text-center">Prüf.</span>
          ...
          <span className="text-[10px] font-bold text-amber-500/80 tracking-tighter text-center">{prepTimeStr}</span>
          
          <span className="text-[10px] font-bold text-cyan-500/60 uppercase tracking-widest truncate">
            {exam.subject}
          </span>

          <span className="text-[10px] font-bold text-cyan-500/80 tracking-tighter text-center">{examTimeStr}</span>
        </div>
