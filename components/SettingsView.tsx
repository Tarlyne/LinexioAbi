...
import { 
  ...
  RefreshCw, Loader2, X, ChevronDown, Tablet, Smartphone, ExternalLink, Eye, EyeOff, XCircle
} from 'lucide-react';
    ...
    <div className="h-full flex flex-col overflow-hidden animate-in fade-in duration-500 w-full relative">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 auto-rows-fr">
          <div className="glass-nocturne border border-slate-700/30 overflow-hidden relative group flex flex-col h-full">
            ...
            <div className="p-6 flex flex-col h-full">
              ...
              <div className="flex-1 flex flex-col gap-4">
                ...
                >
                  <div className="flex items-center justify-center"><FileKey size={24} className="text-cyan-400" /></div>
                  ...
                  <div className="flex items-center justify-center opacity-20 group-hover/btn:opacity-40 transition-all group-hover/btn:scale-110"><FileKey size={28} className="text-slate-500" /></div>
                </button>
                ...
                <button 
                  ...
                  className="btn-secondary-glass p-4 rounded-2xl text-left border-slate-700/50 hover:bg-slate-800/40 group/btn grid grid-cols-[40px_1fr_40px] items-center gap-4 transition-all"
                >
                  <div className="flex items-center justify-center"><Upload size={24} className="text-amber-400" /></div>
                  <div className="flex flex-col min-w-0">
                  ...
                  <div className="flex items-center justify-center opacity-20 group-hover/btn:opacity-40 transition-all group-hover/btn:scale-110"><Upload size={28} className="text-slate-500" /></div>
                </button>
              ...
              <div className="flex-1 flex flex-col gap-6">
                <div className="flex items-center justify-between p-4 bg-slate-900/60 rounded-2xl border border-slate-800/50 shadow-inner shrink-0">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-amber-500/10 flex items-center justify-center border border-amber-500/20 text-amber-500"><Clock size={20} /></div>
                    <div className="flex flex-col">
                ...
                <button onClick={() => setShowChangePwModal(true)} className="btn-secondary-glass p-4 rounded-2xl border-slate-700/50 flex items-center justify-between group flex-1">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-indigo-500/10 flex items-center justify-center border border-indigo-500/20 text-indigo-400 group-hover:bg-indigo-500/20 transition-all"><KeyRound size={20} /></div>
          ...
          <div className="glass-nocturne border border-red-500/20 bg-red-950/5 overflow-hidden relative flex flex-col h-full">
            ...
            <div className="p-6 flex flex-col h-full z-10">
              <h3 className="text-[11px] font-black text-red-500 uppercase tracking-[0.3em] mb-6 flex items-center gap-3 shrink-0"><AlertTriangle size={16} /> Gefahrenzone</h3>
              <div className="flex-1 flex flex-col justify-center items-center gap-6">
                <div className="text-center space-y-2">
                  <h4 className="text-lg font-black text-white tracking-tight">Systembereinigung</h4>
                  <p className="text-[11px] text-slate-400 leading-relaxed max-w-[240px] mx-auto">Daten für ein neues Schuljahr zurücksetzen. Fächer & Master-Passwort bleiben erhalten.</p>
                </div>
                <button onClick={() => setShowResetModal(true)} className="btn-aurora-base btn-danger-aurora px-8 py-4 rounded-2xl text-sm"><RefreshCw size={18} /> <span>Zurücksetzen</span></button>
              </div>
            </div>
          </div>
        </div>
         ...
         <div className="flex flex-col space-y-8 py-2">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-6">
                ...
                <div className="space-y-1">
                   <h3 className="text-2xl font-black text-white tracking-tight">Neues Schuljahr vorbereiten?</h3>
                   <p className="text-sm text-slate-400">Prüfe sorgfältig, welche Daten erhalten bleiben.</p>
                </div>
              </div>
              <button onClick={() => setShowResetModal(false)} className="p-2 text-slate-500 hover:text-white transition-colors -mt-2 -mr-2">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-5 bg-emerald-500/5 border border-emerald-500/20 rounded-2xl space-y-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-black text-emerald-500 uppercase tracking-widest">Bleibt erhalten</span>
                </div>
              ...
              <div className="p-5 bg-red-500/5 border border-red-500/20 rounded-2xl space-y-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-black text-red-500 uppercase tracking-widest">Wird gelöscht</span>
                </div>
                <ul className="space-y-2.5">
                  <li className="flex items-center gap-3 text-sm text-slate-300">
            ...
            <div className="p-4 bg-red-950/20 border border-red-500/20 rounded-xl flex items-start gap-3">
              ...
              <p className="text-[11px] text-red-200/60 leading-relaxed italic">
                Achtung: Dieser Vorgang löscht unwiderruflich alle Planungen des aktuellen Zeitraums. Bitte erstelle vorher ein Backup, falls du die Daten archivieren möchtest.
              </p>
            </div>

            <div className="w-full flex flex-col gap-3">
               <button onClick={() => { resetForNewYear(); setShowResetModal(false); }} className="btn-aurora-base btn-danger-aurora w-full h-14 rounded-2xl text-sm uppercase tracking-widest">
                <RefreshCw size={18} /> Jetzt bereinigen
               </button>
               <button onClick={() => setShowResetModal(false)} className="text-slate-500 hover:text-white text-xs font-bold uppercase py-2 transition-colors">Abbrechen</button>
            </div>
         </div>
      </Modal>
      ...
      <Modal isOpen={showChangePwModal} onClose={() => setShowChangePwModal(false)} maxWidth="max-w-md">
         <form onSubmit={handleChangePw} className="space-y-6">
            <div className="flex items-center justify-between pb-4 border-b border-slate-700/30">
               <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-indigo-500/10 rounded-xl flex items-center justify-center border border-indigo-500/20 text-indigo-400"><KeyRound size={20} /></div>
                  <div><h3 className="text-xl font-black text-white tracking-tight">Passwort ändern</h3><p className="text-[10px] text-indigo-500/80 font-black uppercase tracking-[0.15em]">Sicherheitseinstellungen</p></div>
               </div>
               <button type="button" onClick={() => setShowChangePwModal(false)} className="p-2 text-slate-500 hover:text-white transition-colors"><X size={20} /></button>
            </div>
            <div className="space-y-4">
               <div className="space-y-1.5">
                  <label className="text-[10px] uppercase font-bold text-slate-500 ml-1">Aktuelles Passwort</label>
                  <div className="relative">
                    <input autoFocus type={showPws.old ? "text" : "password"} placeholder="Aktuelles Passwort" value={oldPw} onChange={e => setOldPw(e.target.value)} className="w-full bg-slate-950 border border-slate-800 p-4 pr-12 rounded-xl text-white focus:ring-1 focus:ring-indigo-500/40" />
                    <button type="button" tabIndex={-1} onPointerUp={() => setShowPws(prev => ({...prev, old: !prev.old}))} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 text-slate-500 hover:text-white transition-colors">
               ...
               <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <input type={showPws.new ? "text" : "password"} placeholder="Neues Passwort" value={newPw} onChange={e => setNewPw(e.target.value)} className="w-full bg-slate-950 border border-slate-800 p-4 pr-12 rounded-xl text-white focus:ring-1 focus:ring-emerald-500/40" />
                    <button type="button" tabIndex={-1} onPointerUp={() => setShowPws(prev => ({...prev, new: !prev.new}))} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 text-slate-500 hover:text-white transition-colors">
               ...
               <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <input type={showPws.confirm ? "text" : "password"} placeholder="Wiederholen" value={confirmPw} onChange={e => setConfirmPw(e.target.value)} className="w-full bg-slate-950 border border-slate-800 p-4 pr-12 rounded-xl text-white focus:ring-1 focus:ring-emerald-500/40" />
                    <button type="button" tabIndex={-1} onPointerUp={() => setShowPws(prev => ({...prev, confirm: !prev.confirm}))} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 text-slate-500 hover:text-white transition-colors">
      ...
      <Modal isOpen={showExportModal} onClose={() => setShowExportModal(false)} maxWidth="max-w-md">
        <form onSubmit={handleExport} className="space-y-6">
           <div className="flex items-center justify-between pb-4 border-b border-slate-700/30">
             <div className="flex items-center gap-4">
                <div className="w-10 h-10 bg-cyan-500/10 rounded-xl flex items-center justify-center border border-cyan-500/20 text-cyan-400"><FileKey size={20} /></div>
                <div><h3 className="text-xl font-black text-white tracking-tight">Export verschlüsseln</h3></div>
             </div>
             <button type="button" onClick={() => setShowExportModal(false)} className="p-2 text-slate-500 hover:text-white transition-colors"><X size={20} /></button>
           </div>
           <div className="space-y-4">
             <div className="space-y-1.5">
               <label className="text-[10px] uppercase font-bold text-slate-500 ml-1">Sicherheits-Passwort</label>
               <div className="relative">
                 <input autoFocus type={showBackupPw ? "text" : "password"} value={backupPassword} onChange={e => setBackupPassword(e.target.value)} className="w-full bg-slate-950 border border-slate-800 p-4 pr-12 rounded-xl text-white focus:ring-1 focus:ring-cyan-500/40" placeholder="Passwort für diese Datei" />
                 <button type="button" tabIndex={-1} onPointerUp={() => setShowBackupPw(!showBackupPw)} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 text-slate-500 hover:text-white transition-colors">
             ...
             <div className="space-y-1.5">
               ...
               <input type={showBackupPw ? "text" : "password"} value={confirmBackupPassword} onChange={e => setConfirmBackupPassword(e.target.value)} className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl text-white focus:ring-1 focus:ring-cyan-500/40" placeholder="Erneut eingeben" />
             </div>
           </div>
           <button disabled={isProcessing || !backupPassword || !confirmBackupPassword} className="btn-aurora-base btn-primary-aurora w-full h-14 rounded-2xl font-black uppercase tracking-widest disabled:opacity-30">Backup generieren</button>
        </form>
      </Modal>
      ...
      <Modal isOpen={showImportModal} onClose={() => setShowImportModal(false)} maxWidth="max-w-md">
        <form onSubmit={async e => { e.preventDefault(); setIsProcessing(true); const ok = await importState(selectedFile!, backupPassword); setIsProcessing(false); if(ok) setShowImportModal(false); }} className="space-y-6">
           <div className="flex items-center justify-between pb-4 border-b border-slate-700/30">
             <div className="flex items-center gap-4">
                <div className="w-10 h-10 bg-amber-500/10 rounded-xl flex items-center justify-center border border-amber-500/20 text-amber-400"><Upload size={20} /></div>
                <div><h3 className="text-xl font-black text-white tracking-tight">Backup einspielen</h3></div>
             </div>
             <button type="button" onClick={() => setShowImportModal(false)} className="p-2 text-slate-500 hover:text-white transition-colors"><X size={20} /></button>
           </div>
           <div className="relative">
             <input autoFocus type={showBackupPw ? "text" : "password"} value={backupPassword} onChange={e => setBackupPassword(e.target.value)} className="w-full bg-slate-950 border border-slate-800 p-4 pr-12 rounded-xl text-white focus:ring-1 focus:ring-amber-500/40" placeholder="Datei-Passwort eingeben" />
             <button type="button" tabIndex={-1} onPointerUp={() => setShowBackupPw(!showBackupPw)} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 text-slate-500 hover:text-white transition-colors">
               {showBackupPw ? <EyeOff size={18} /> : <Eye size={18} />}
             </button>
           </div>
           <button disabled={isProcessing || !backupPassword} className="btn-aurora-base btn-warning-aurora w-full h-14 rounded-xl font-black uppercase tracking-widest disabled:opacity-30">Entschlüsseln & Laden</button>
        </form>
      </Modal>
    </div>
  );
