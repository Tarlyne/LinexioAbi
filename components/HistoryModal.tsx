...
export const HistoryModal: React.FC<HistoryModalProps> = ({ isOpen, onClose, logs }) => {
  ...
  const getIcon = (type: HistoryLog['type']) => {
    switch (type) {
      case 'create': return <Plus size={14} className="text-emerald-400" />;
      case 'update': return <Edit2 size={14} className="text-cyan-400" />;
      case 'delete': return <Trash2 size={14} className="text-red-400" />;
      default: return <Shield size={14} className="text-indigo-400" />;
    }
  };
  ...
  const getBg = (type: HistoryLog['type']) => {
    switch (type) {
      ...
      case 'update': return 'bg-cyan-500/10 border-cyan-500/20';
    ...
    <Modal isOpen={isOpen} onClose={onClose} maxWidth="max-w-2xl">
      <div className="flex flex-col gap-6">
        ...
        <div className="flex-1 overflow-y-auto max-h-[60vh] space-y-3 no-scrollbar pr-1">
          {logs.length === 0 ? (
            <div className="py-20 text-center text-slate-600 italic">Noch keine Aktionen protokolliert.</div>
          ) : (
            logs.map((log) => (
              <div key={log.id} className="group relative flex gap-4 p-4 bg-slate-900/40 border border-slate-800 rounded-2xl transition-all hover:bg-slate-900/60 hover:border-slate-700">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border ${getBg(log.type)}`}>
                  {getIcon(log.type)}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-4 mb-1">
                    ...
                    <span className="text-[10px] font-mono text-slate-500 shrink-0">{formatTime(log.timestamp)}</span>
                  </div>
        ...
        <div className="pt-4 border-t border-slate-700/30 flex justify-end">
          <button onClick={onClose} className="btn-secondary-glass px-8 h-11 rounded-xl text-xs uppercase tracking-widest font-bold">Schließen</button>
        </div>
      </div>
    </Modal>
  );
