...
export const BacklogSidebar: React.FC<BacklogSidebarProps> = ({
  ...
  sortOption, onSortChange, onEditExam, 
  ...
  useEffect(() => { onRemoveRef.current = onRemoveFromGrid; }, [onRemoveFromGrid]);
    ...
    >
      <div className="p-4 border-b border-slate-700/30 space-y-4 shrink-0">
        ...
        <div className="relative flex p-1 bg-slate-900/50 rounded-lg border border-slate-700/30">
          <div className="absolute top-1 bottom-1 left-1 bg-slate-700/80 rounded-md shadow-sm transition-all duration-300 ease-in-out" style={{ width: `calc((100% - 8px) / ${sortOptions.length})`, transform: `translateX(calc(${sortOptions.indexOf(sortOption)} * 100%))` }} />
          {sortOptions.map((option) => (
            <button key={option} onClick={() => onSortChange(option)} className={`relative z-10 flex-1 flex items-center justify-center py-1.5 rounded-md text-[10px] font-bold uppercase tracking-widest transition-colors duration-300 ${sortOption === option ? 'text-white' : 'text-slate-500 hover:text-slate-300'}`}>
        ...
        <div className="relative">
          ...
          <input type="text" placeholder="Suche..." value={searchTerm} onChange={e => onSearchChange(e.target.value)} className="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl pl-9 pr-3 py-2 text-xs text-white focus:ring-1 focus:ring-cyan-500/40" />
        </div>
      </div>
          ...
          return (
            <div key={exam.id} onPointerDown={(e) => { if (e.button !== 0) return; startDrag(exam.id, 'exam', e, { fromBacklog: true }, ghostUI); }} className={`draggable-item p-3 border rounded-xl hover:border-cyan-500/40 transition-all group relative flex flex-col ${isDraggingReal ? 'opacity-20 scale-95 border-cyan-500' : 'opacity-100'} ${hasWarning ? 'bg-amber-900/20 border-amber-500/30' : isNakedDraft ? 'bg-red-500/5 border-red-500/20' : 'bg-slate-800/40 border-slate-700/50'}`}>
              <div className="flex justify-between items-start mb-1">
                ...
                <button onPointerDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); onEditExam(exam); }} className="w-6 h-6 -mr-1 -mt-1 rounded-lg flex items-center justify-center text-slate-600 hover:text-white hover:bg-white/5 active:scale-90 transition-all pointer-events-auto"><Settings size={14} /></button>
              </div>
              ...
              <div className="grid grid-cols-3 gap-1 mt-auto pt-1.5 border-t border-slate-700/30 pointer-events-none overflow-hidden">
                <div className="flex justify-center"><div className={`bg-cyan-500/10 border border-cyan-500/20 px-2 py-0.5 rounded text-[9px] font-bold text-center min-w-[34px] ${isNakedDraft ? 'text-slate-600' : 'text-cyan-300'}`}>{teacher?.shortName || '?'}</div></div>
                <div className="flex justify-center"><div className={`px-2 py-0.5 rounded text-[9px] font-bold text-center border transition-all min-w-[34px] ${protocol ? 'bg-indigo-500/10 border-indigo-500/20 text-indigo-300' : 'border-transparent text-transparent'}`}>{protocol?.shortName || '---'}</div></div>
                <div className="flex justify-center"><div className={`px-2 py-0.5 rounded text-[9px] font-bold text-center border transition-all min-w-[34px] ${chair ? 'bg-amber-500/10 border-amber-500/20 text-amber-300' : 'border-transparent text-transparent'}`}>{chair?.shortName || '---'}</div></div>
              </div>
            </div>
          );
        })}
      ...
      <div className="bg-slate-900/60 border-t border-slate-700/30 px-4 py-2 flex justify-between items-center shrink-0">
        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{exams.length} Einträge</span>
      </div>
    </aside>
  );
};
