...
import { 
  ...
  MapPin, User, GraduationCap, Shield, FileText, Info
} from 'lucide-react';
...
export const ImportInstructionsModal: React.FC<ImportInstructionsModalProps> = ({ isOpen, onClose, onSelectFile }) => {
  const columns = [
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="bg-indigo-500/5 border border-indigo-500/20 rounded-2xl p-4 flex gap-4">
          ...
          <p className="text-xs text-slate-300 leading-relaxed">
            Bitte stelle sicher, dass deine Excel-Datei als <strong>CSV (Trennzeichen-getrennt)</strong> mit Semikolon (;) gespeichert ist. Die Reihenfolge der 9 Spalten muss exakt wie folgt sein:
          </p>
        </div>

        <div className="grid grid-cols-3 gap-3">
          {columns.map((col, idx) => (
            <div key={idx} className="relative p-3 bg-slate-900/40 border border-slate-800 rounded-xl flex flex-col items-center text-center gap-1.5 pt-6 group">
              {/* Nummerierungs-Badge */}
              ...
              <col.icon size={14} className="text-indigo-400 opacity-60 group-hover:opacity-100 transition-opacity" />
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{col.label}</span>
              <span className="text-[9px] font-mono text-slate-600 truncate w-full">{col.example}</span>
            </div>
          ))}
        </div>
