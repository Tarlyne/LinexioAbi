...
import { 
  ...
  MapPin, Loader2, Info, ChevronDown, Calendar, BookOpen
} from 'lucide-react';
...
export const ExamImportWizard: React.FC<ExamImportWizardProps> = ({ isOpen, onClose, rawData }) => {
  ...
  const unmappedRows = useMemo(() => {
    return mappingEntries.filter(([_, m]) => {
      ...
      return !!m.error || studentMissing || examinerMissing || protocolMissing || chairMissing || examRoomMissing || prepRoomMissing || subjectMissing;
    });
  }, [mappingEntries]);
  ...
  const handleImport = () => {
    const examsToImport: Exam[] = (Object.values(mappings) as MappingResult[])
      .filter(m => {
        ...
        const optionalValid = (!m.row.prepRoom || m.prepRoomId) && (!m.row.protocol || m.protocolId) && (!m.row.chair || m.chairId);
      ...
      <div className="flex flex-col h-[85vh]">
        <div className="flex items-center justify-between pb-4 border-b border-slate-700/30 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-lg font-bold text-white tracking-tight">Interaktiver Import-Wizard</h3>
        ...
        <div className="flex-1 overflow-y-auto py-6 space-y-4 no-scrollbar">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  {missingData.dates.length > 0 && (
                    <div className="space-y-1">
                      <p className="text-[10px] uppercase font-black text-slate-500 tracking-widest">Unbekannte Tage:</p>
                      <p className="text-xs text-white font-medium">{missingData.dates.join(', ')}</p>
                    </div>
                  )}
                  {missingData.rooms.length > 0 && (
                    <div className="space-y-1">
                      <p className="text-[10px] uppercase font-black text-slate-500 tracking-widest">Unbekannte Räume:</p>
                      <p className="text-xs text-white font-medium">{missingData.rooms.join(', ')}</p>
                    </div>
                  )}
                  {missingData.teachers.length > 0 && (
                    <div className="space-y-1">
                      <p className="text-[10px] uppercase font-black text-slate-500 tracking-widest">Unbekannte Lehrer:</p>
                      <p className="text-xs text-white font-medium">{missingData.teachers.join(', ')}</p>
                    </div>
                  )}
                  {missingData.subjects.length > 0 && (
                    <div className="space-y-1">
                      <p className="text-[10px] uppercase font-black text-slate-500 tracking-widest">Unbekannte Fächer:</p>
                      <p className="text-xs text-white font-medium">{missingData.subjects.join(', ')}</p>
                    </div>
                  )}
                </div>
                <p className="text-[11px] text-amber-500/80 font-medium italic border-t border-amber-500/20 pt-2">
                  Tipp: Korrigiere die Daten manuell in den Zeilen oder lege die fehlenden Elemente in der Datenbank an.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-3">
            {mappingEntries.map(([key, m]) => {
              ...
              const isRowError = !!m.error || studentErr || examinerErr || protocolErr || chairErr || roomErr || prepRoomErr || subjectErr;
              
              return (
                <div key={key} className={`p-4 rounded-2xl border flex flex-col gap-5 transition-all ${
                  isRowError ? 'bg-red-500/5 border-red-500/20' : 'bg-slate-900/40 border-slate-800'
                }`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] font-mono text-slate-500 bg-slate-800 px-2 py-0.5 rounded">ZEILE {idx + 1}</span>
                      ...
                      <div className="relative">
                        <select 
                          ...
                          className={`w-full h-10 appearance-none bg-slate-950 border rounded-xl px-3 text-xs transition-all ${
                            m.studentId ? 'border-slate-800 text-white' : 'border-red-500/50 text-red-400 bg-red-500/5'
                          }`}
                        >
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        <select 
                          ...
                          className={`w-full h-10 appearance-none bg-slate-950 border rounded-xl px-3 text-xs transition-all ${
                            m.subjectId ? 'border-slate-800 text-white font-bold' : 'border-red-500/50 text-red-400 bg-red-500/5'
                          }`}
                        >
                          <option value="">-- Wählen --</option>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        <select 
                          ...
                          className={`w-full h-10 appearance-none bg-slate-950 border rounded-xl px-3 text-xs transition-all ${
                            m.examinerId ? 'border-slate-800 text-white' : 'border-red-500/50 text-red-400 bg-red-500/5'
                          }`}
                        >
                          <option value="">-- Wählen --</option>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        <select 
                          ...
                          className={`w-full h-10 appearance-none bg-slate-950 border rounded-xl px-3 text-xs transition-all ${
                            m.protocolId ? 'border-slate-800 text-white' : m.row.protocol ? 'border-red-500/50 text-red-400 bg-red-500/5' : 'border-slate-800 text-slate-500'
                          }`}
                        >
                          <option value="">-- Wählen --</option>
                          {teachers.map(t => <option key={t.id} value={t.id}>{t.shortName}</option>)}
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        <select 
                          ...
                          className={`w-full h-10 appearance-none bg-slate-950 border rounded-xl px-3 text-xs transition-all ${
                            m.chairId ? 'border-slate-800 text-white' : m.row.chair ? 'border-red-500/50 text-red-400 bg-red-500/5' : 'border-slate-800 text-slate-500'
                          }`}
                        >
                          <option value="">-- Wählen --</option>
                          {teachers.map(t => <option key={t.id} value={t.id}>{t.shortName}</option>)}
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        <select 
                          ...
                          className={`w-full h-10 appearance-none bg-slate-950 border rounded-xl px-3 text-xs transition-all ${
                            m.roomId ? 'border-slate-800 text-white' : 'border-red-500/50 text-red-400 bg-red-500/5'
                          }`}
                        >
                          <option value="">-- Wählen --</option>
        ...
        <div className="pt-6 border-t border-slate-700/30 flex items-center justify-between shrink-0">
          <div className="flex flex-col">
            ...
            <span className={`text-[11px] font-bold ${unmappedRows.length > 0 ? 'text-amber-500' : 'text-emerald-500'}`}>
              {unmappedRows.length > 0 ? `${unmappedRows.length} Korrekturen erforderlich` : 'Alle Daten valide'}
            </span>
          </div>
