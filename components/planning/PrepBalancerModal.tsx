...
import { BookOpen, MapPin, ChevronRight, CheckCircle2, AlertTriangle, Info, Clock } from 'lucide-react';
...
export const PrepBalancerModal: React.FC<PrepBalancerModalProps> = ({ 
  isOpen, onClose, dayExams, prepRooms, onApply 
}) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} maxWidth="max-w-4xl">
      <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8 overflow-hidden">
          {/* Linke Seite: Fächer-Liste */}
          <div className="space-y-4 overflow-y-auto max-h-[50vh] pr-2 no-scrollbar">
            <div className="flex items-center gap-2 mb-2">
              <BookOpen size={14} className="text-slate-500" />
              <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Fächer am heutigen Tag</span>
            </div>
            {daySubjects.map(subject => {
              const examCount = dayExams.filter(e => e.subject === subject).length;
              return (
                <div key={subject} className="p-3 bg-slate-900/40 border border-slate-800 rounded-xl flex items-center justify-between group transition-all hover:border-slate-700">
                  <div className="flex flex-col">
                    <span className="text-sm font-bold text-slate-200">{subject}</span>
                    <span className="text-[10px] text-slate-500 font-medium">{examCount} Prüfung{examCount === 1 ? '' : 'en'}</span>
                  </div>
          ...
          {/* Rechte Seite: Prognose / Heatmap Ersatz */}
          <div className="space-y-6 flex flex-col h-full">
            <div className="flex items-center gap-2 mb-2">
              <MapPin size={14} className="text-slate-500" />
              <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Auslastungs-Prognose</span>
            </div>
            ...
            <div className="space-y-4 flex-1">
              {prepRooms.map(room => {
                ...
                return (
                  <div key={room.id} className={`p-4 rounded-2xl border transition-all ${
                    isOverloaded ? 'bg-red-500/10 border-red-500/30' : 
                    isWarning ? 'bg-amber-500/10 border-amber-500/30' : 
                    'bg-slate-900/60 border-slate-800'
                  }`}>
                    <div className="flex justify-between items-start mb-4">
                      <span className="text-xs font-black text-white uppercase tracking-wider">{room.name}</span>
                      {isOverloaded ? <AlertTriangle size={16} className="text-red-500 animate-pulse" /> : 
                       isWarning ? <Info size={16} className="text-amber-500" /> : 
                       <CheckCircle2 size={16} className="text-emerald-500 opacity-60" />}
                    </div>

                    <div className="flex items-end justify-between">
                      <div className="flex flex-col">
                        <span className="text-[10px] text-slate-500 font-bold uppercase mb-1">Max. Peak</span>
                        <span className={`text-3xl font-black ${isOverloaded ? 'text-red-400' : isWarning ? 'text-amber-400' : 'text-emerald-400'}`}>
                          {stat.peak}
                        </span>
                      </div>
                      <div className="flex flex-col text-right">
                        <span className="text-[10px] text-slate-500 font-bold uppercase mb-1">Zeitpunkt</span>
                        <span className="text-sm font-mono font-bold text-white">{stat.time} Uhr</span>
                      </div>
                    </div>
            ...
            <div className="p-4 bg-slate-950/40 rounded-2xl border border-slate-800 space-y-2">
               <p className="text-[10px] text-slate-500 leading-relaxed italic">
                 Berechnung basiert auf dem 20-Minuten Vorbereitungs-Zeitraum vor jedem Prüfungsbeginn. Peak-Empfehlung: max. 6 Personen.
               </p>
            </div>
          </div>
        </div>
        ...
        <div className="pt-6 border-t border-slate-700/30 flex gap-4">
          <button 
            onClick={onClose} 
            className="btn-secondary-glass flex-1 h-12 rounded-xl text-sm"
          >
            Abbrechen
          </button>
          <button 
