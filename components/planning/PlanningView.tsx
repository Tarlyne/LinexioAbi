...
import { 
  MapPin, Printer, Upload, Layers, ChevronDown, UserCheck, RotateCcw
} from 'lucide-react';
...
export const PlanningView: React.FC = () => {
  ...
  const handleCsvFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (rows.length === 0) { showToast('Die Datei scheint leer zu sein.', 'warning'); return; }
        setImportRawData(rows);
        setShowImportWizard(true);
      } catch (err) { showToast('Fehler beim Lesen der CSV-Datei.', 'error'); }
    };
  ...
  const openEditModal = useCallback((exam: Exam, fromManager = false) => {
    setEditingExam({ ...exam });
  ...
  const handleSaveExam = (e: React.FormEvent, applyToGroup: boolean = false) => {
    ...
    if (applyToGroup && savedExam.groupId) {
      const siblings = exams.filter(ex => ex.id !== savedExam.id && ex.groupId === savedExam.groupId && ex.subject === savedExam.subject && ex.teacherId === savedExam.teacherId);
      if (siblings.length > 0) {
        siblings.forEach(s => updateExam({ ...s, chairId: savedExam.chairId, protocolId: savedExam.protocolId, prepRoomId: savedExam.prepRoomId }));
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => { if (actionsRef.current && !actionsRef.current.contains(e.target as Node)) setIsActionsOpen(false); };
  ...
  useHeader(
    <div className="flex items-center gap-2">
      <button onClick={() => undo()} disabled={!canUndo} className={`btn-secondary-glass w-9 h-9 p-0 rounded-xl shadow-lg border-cyan-500/30 transition-all ${canUndo ? 'text-cyan-400' : 'text-slate-600 opacity-30 cursor-not-allowed'}`} title="Rückgängig (Cmd+Z)"><RotateCcw size={15} /></button>
      <div className="relative" ref={actionsRef}>
        <button onClick={() => setIsActionsOpen(!isActionsOpen)} className={`btn-aurora-base h-9 px-4 rounded-xl text-[11px] uppercase tracking-wider transition-all border shadow-lg ${isActionsOpen ? 'bg-cyan-500 text-white border-cyan-400' : 'border-cyan-500/50 text-cyan-400 bg-cyan-500/5 hover:bg-cyan-500/10'}`}>
          <span>Aktionen</span>
        ...
        {isActionsOpen && (
          <div className="absolute right-0 mt-2 w-64 glass-modal border border-slate-700/50 p-1.5 shadow-2xl z-[100] animate-in fade-in slide-in-from-top-2 duration-200">
            <button onClick={() => { setShowExamManager(true); setIsActionsOpen(false); }} className="w-full flex items-center gap-3 p-3 rounded-xl text-slate-300 hover:bg-cyan-500/10 hover:text-cyan-400 transition-all text-left">
              <UserCheck size={18} className="shrink-0 text-cyan-400" />
              <div className="flex flex-col"><span className="text-xs font-bold leading-none">Prüfungs-Manager</span><span className="text-[9px] text-slate-500 font-medium mt-1">Matrix & Personenzentriert</span></div>
            </button>
            <button onClick={() => { setShowPrepBalancer(true); setIsActionsOpen(false); }} className="w-full flex items-center gap-3 p-3 rounded-xl text-slate-300 hover:bg-amber-500/10 hover:text-amber-400 transition-all text-left">
              ...
              <div className="flex flex-col"><span className="text-xs font-bold leading-none">Vorb.-räume planen</span><span className="text-[9px] text-slate-500 font-medium mt-1">Bulk-Zuweisung Räume</span></div>
            </button>
            ...
            <button onClick={() => { setShowPrintPreview(true); setIsActionsOpen(false); }} className="w-full flex items-center gap-3 p-3 rounded-xl text-slate-300 hover:bg-cyan-500/10 hover:text-cyan-400 transition-all text-left">
  ...
  return (
    <div className="flex flex-col h-full gap-4 overflow-hidden select-none print:hidden">
      ...
      <ExamEditorModal isOpen={showModal} onClose={handleModalClose} editingExam={editingExam} setEditingExam={setEditingExam} students={students} teachers={teachers} rooms={rooms} subjects={subjects} onSave={handleSaveExam} onDelete={(id) => { deleteExam(id); setShowModal(false); if (returnToManager) setShowExamManager(true); }} showDeleteConfirm={showDeleteConfirm} setShowDeleteConfirm={setShowDeleteConfirm} />
      ...
      <ExamManagerModal isOpen={showExamManager} onClose={() => setShowExamManager(false)} onEditExam={(exam) => openEditModal(exam, true)} />
      
      <PlanningExportModal isOpen={showPrintPreview} onClose={() => setShowPrintPreview(false)} activeDayIdx={activeDay} appState={{ teachers, students, rooms, days, subjects, exams, supervisions, historyLogs, collectedExamIds, isLocked: false, masterPassword: null, settings: { autoLockMinutes: 10 }, lastUpdate: Date.now() }} />
