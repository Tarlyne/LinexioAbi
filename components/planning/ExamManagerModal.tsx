...
import { Search, X, UserCheck, BookOpen, Clock, SearchX, Plus, Check, AlertTriangle } from 'lucide-react';
...
export const ExamManagerModal: React.FC<ExamManagerModalProps> = ({ isOpen, onClose, onEditExam }) => {
  ...
  const studentData = useMemo(() => {
    return students.map(student => {
      ...
      return {
        ...student,
        exams: studentExams,
        searchPool: [student.lastName, student.firstName, ...studentExams.map(e => {
  ...
  const renderDateTime = (startTime: number) => {
    if (startTime === 0) return <span className="text-red-500 font-bold uppercase text-[8px]">Nicht geplant</span>;
    ...
    return <span className="text-cyan-400 font-bold">{dateStr ? formatDateShort(dateStr) : 'T' + (dIdx + 1)}, {h}:{m}</span>;
  };
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-700/30 shrink-0">
          ...
          <div className="flex-1 flex justify-center px-8">
            <div className="relative w-full max-w-xl">
              ...
              <input autoFocus type="text" placeholder="Schüler oder Fach suchen..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl pl-9 pr-8 py-2.5 text-xs text-white focus:ring-1 focus:ring-cyan-500/40" />
          ...
          <div className="flex items-center gap-3">
             <button onClick={onClose} className="p-2 text-slate-500 hover:text-white transition-colors ml-2"><X size={20} /></button>
          </div>
        </div>
                        ...
                        <button onClick={() => onEditExam(exam)} className={`w-full text-left p-3 rounded-xl border transition-all hover:scale-[1.02] active:scale-95 shadow-sm space-y-1.5 ${
                          ...
                          <div className="flex items-center justify-between pt-1.5 border-t border-slate-800/60 mt-1">
                            <div className="flex items-center gap-1.5 min-w-0">
                               ...
                               <span className="text-[9px] truncate">{renderDateTime(exam.startTime)}</span>
                            </div>
                            ...
                            <div className="flex items-center shrink-0">
                              {hasWarning ? (
                                ...
                                !isNakedDraft && <AlertTriangle size={12} className="text-red-500" />
                              )}
                            </div>
                          </div>
                        </button>
                ...
                <tr>
                  <td colSpan={maxExamsCount + 2} className="px-6 py-24 text-center">
                    <div className="flex flex-col items-center justify-center gap-4 text-slate-600">
                      ...
                      <div className="space-y-1"><p className="text-lg font-bold">Keine Treffer</p><p className="text-sm italic">Verfeinere deine Suche.</p></div>
                    </div>
                  </td>
                </tr>
              )}
